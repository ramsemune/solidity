...
namespace
{
bool functionContainsCall(Dialect const& _dialect, map<YulString, SideEffects> const& _functionSideEffects, FunctionCall const& _funCall) {
	...
	if (auto const* sideEffects = util::valueOrNullptr(_functionSideEffects, _funCall.functionName.name))
		if (!sideEffects->keepsReturndatasize)
			return true;
	return false;
}
...
public:
	...
	void operator()(FunctionCall& _funCall) override
	{
		...
		if (functionContainsCall(m_dialect, m_functionSideEffects, _funCall))
			m_seenCall = true;
	}
	void operator()(ForLoop& _loop) override
	{
		bool hadSeenCall = m_seenCall;
...
void ZeroByReturndatasizeReplacer::visit(Expression& _e)
{
	if (
		...
		!m_seenCall && literal && valueOfLiteral(*literal) == 0
	)
		_e = FunctionCall{
			literal->location,
